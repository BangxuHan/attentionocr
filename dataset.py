...
dataset_path = {  'art': os.path.join(base_dir, 'art/train_task2_images'), 
                  ...
                  'mls': os.path.join(base_dir, 'train'),
        ...
        for filename in os.listdir(label_folder):
            ...
            with open(os.path.join(label_folder, filename)) as f:
                ...
                for polygon, transcript, ignore in zip(points, transcripts, ignores):
                    ...
                    #if bbox_w <8 or bbox_h <8 or bbox_h > 2 * bbox_w:
...
class Mlsdata(Dataset):
    """
    mls
    """

    def __init__(self, name='mls'):
        super(Mlsdata, self).__init__(name=name)
        self.transcripts = []
...
if __name__=='__main__':
    ...
    filenames   = LSVT.filenames + ART.filenames + ReCTS.filenames + mls.filenames
    labels      = LSVT.labels    + ART.labels    + ReCTS.labels    + mls.labels
    masks       = LSVT.masks     + ART.masks     + ReCTS.masks     + mls.masks
    bboxes      = LSVT.bboxes    + ART.bboxes    + ReCTS.bboxes    + mls.bboxes
    points      = LSVT.points    + ART.points    + ReCTS.points    + mls.points

    #filenames = mls.filenames+ ART.filenames
    # labels = mls.labels+ ART.labels
    # masks = mls.masks+ ART.masks
    # bboxes = mls.bboxes+ ART.bboxes
    # points = mls.points+ ART.points
    #
