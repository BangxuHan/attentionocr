...
from dataset import LSVT, ART, ReCTS, Mlsdata #, TotalText, ICDAR2017RCTW, MLT2019
...
def padding_image_kl(image, padding_size):
    """
    Padding arbitrary-shaped text image to square for tensorflow batch training.
    """
    height, width = image.shape[:2]
...
def cv2_letterbox_image(image, expected_size):
    ...
    top = (eh - nh)
    if top > 0:
        top = np.random.randint(top)
    bottom = nh + top
    left = (ew - nw)
    if left > 0:
        left = np.random.randint(left)
    right = nw + left
    new_img[top:bottom, left:right, :] = image
    #new_img = new_img / 255.
    #new_img = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT)
    return new_img, (top, left, nh, nw)
...
class TextDataPreprocessor:
    ...
    def __call__(self, roidb):
        ...
        #img, crop_bbox = padding_image_kl(img, (100,cfg.image_size))
        img, crop_bbox = cv2_letterbox_image(img, (cfg.image_height,cfg.image_size))
